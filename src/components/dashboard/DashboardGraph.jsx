...
const DashboardChart = () => {
  ...
  const myCourses = useMemo(() => {
    return allCourses?.data?.filter((course) => course.teacher._id === userId) || [];
...
shadow-lg transition-all duration-300 hover:shadow-xl hover:scale-[1.01]">
      <div className="px-6 pt-5 pb-2 border-b border-gray-200/50 dark:border-gray-700">
    <div className="flex items-center justify-between">

      <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-100 flex items-center">
        <FiTrendingUp className="mr-2 text-blue-500 dark:text-blue-400" />
        Course Analytics
      </h3>
      ...
      <div className="flex space-x-2">
        ...
        <button
          className={`p-1.5 rounded-lg border border-gray-200 dark:border-gray-600 transition-colors ${
            chartType === 'area'
    ...
    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
      ...
      <div
        className="px-6 py-3 border-t border-gray-200/50 dark:border-gray-700 
bg-white/50 dark:bg-gray-900/40 flex justify-between items-center"
      >
        <div className="text-sm">
          <span className="text-gray-500 dark:text-gray-400">Total Courses:</span>

          <span className="font-medium text-blue-600 dark:text-blue-400 ml-1">{myCourses.length}</span>
        </div>

        <div className="flex items-center text-sm text-gray-500 dark:text-gray-400">Updated in real-time</div>
      </div>
    </div>
  );
};
